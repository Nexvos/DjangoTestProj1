...
def get_match_data_by_tournament_id(tournament_id):
    ...
    tournaments_endpoint = "series/" + tournament_id + "/tournaments.json?"
    matches_endpoint = "/tournaments/%s/matches.json?"
    optional_filter = "filter[name]=Overwatch&"
    api_key = "token=" + getApiKey()
...
def update_existing_tournaments():
    ...
    for tournament in tournaments:
        ...
        if tournament.status != tournament.finished:
            ...
            data = get_match_data_by_tournament_id(tournament.api_tournament_id)
            ...
            print("    database modified date: " + tournament.api_modified_at)
            print("    API modified date: " + tournament_modified_datetime)
            #update tournment details if modified date is more recent than saved modified date
            if tournament_modified_datetime > tournament.api_modified_at:
                ...
                tournament_start_date = datetime.strptime(
                    data["tournament_start_datetime"],
                    '%Y-%m-%dT%H:%M:%SZ'
                )
                # datetime on the API is converted to a python datetime for calculations below
                tournament_end_date = datetime.strptime(
                    data["tournament_end_datetime"],
                    '%Y-%m-%dT%H:%M:%SZ'
                )
            ...
            for game in games:
                ...
                for api_game in data["matches"]:
                    ...
                    if game.api_match_id == api_game["match_id"] and match_modified_datetime > game.api_modified_at:
